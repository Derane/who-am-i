...
class ClientPlayerIT {
    ...
    @Test
    void clientReadsPlayersNameFromSocket() throws IOException, InterruptedException, TimeoutException, ExecutionException {
        InetAddress localHost = InetAddress.getLocalHost();
        int port = randomPort();
        ...
        try (ServerSocket server = new ServerSocket()) {
            ...
            try (Socket client = server.accept();
                ...
                boolean success = clientReady.await(5, TimeUnit.SECONDS);
                assertTrue(success);
                String character = player.setName().get(5, TimeUnit.SECONDS);
